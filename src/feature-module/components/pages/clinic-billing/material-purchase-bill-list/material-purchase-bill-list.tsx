...
const api = axios.create({
  baseURL: "http://3.109.62.26/api/billing/clinic/material-purchase/",
});
...
const MaterialPurchaseBills = () => {
  ...
  // ✅ Fetch bills (Superadmin-compatible)
  useEffect(() => {
    const token = localStorage.getItem("access_token");
    if (!token) {
  ...
  const fetchBills = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch material purchase bills");
      }
  ...
  // ✅ Delete Bill (Superadmin-compatible)
  const handleDelete = async () => {
    ...
    try {
      ...
      let apiUrl = `http://3.109.62.26/api/billing/clinic/material-purchase/${deleteId}/`;
      if (clinicId) {
        apiUrl += `?clinic_id=${clinicId}`;
      }
      ...
      if (!response.ok) {
        throw new Error("Failed to delete material purchase bill");
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (supplier_name: string) => supplier_name || "N/A",
    },
    {
      title: "Invoice Number",
      dataIndex: "invoice_number",
      render: (invoice_number: string) => invoice_number || "N/A",
    },
    {
      title: "Clinic",
      dataIndex: "clinic",
      ...
      render: (record: any) => (
        <div className="action-item">
          ...
          <ul className="dropdown-menu p-2">
            <li>
              <Link
                to={`/clinic-dashboard/edit-material-purchase-bill/${record.id}`}
            ...
            <li>
              <Link
                to={`/clinic-dashboard/view-material-purchase-bill/${record.id}`}
                className="dropdown-item d-flex align-items-center"
              >
                View
              </Link>
  ...
  const exportExcel = () => {
    ...
    // ✅ Decide which data to export (filtered or all)
    ...
    const fileName = hasFilters
      ? "MaterialPurchaseBills_Filtered.xlsx"
      : "MaterialPurchaseBills_All.xlsx";
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Material Purchase Bills");

  const filename = `${clinicName}_Material_Purchase_Bills.xlsx`;
        ...
        <div className="content">
          ...
          <div className="d-flex align-items-sm-center flex-sm-row flex-column gap-2 mb-3 pb-3 border-bottom">
            ...
            <div className="text-end d-flex">
              <Link
                to="/clinic-dashboard/add-material-purchase-bill"
...
export default MaterialPurchaseBills;
