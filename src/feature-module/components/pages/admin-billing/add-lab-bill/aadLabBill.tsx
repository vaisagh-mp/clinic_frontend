...
const AddLabBill = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(
        "http://3.109.62.26/api/billing/admin/lab-bill/",
        formData,
        {
          headers: {
      ...
      navigate(all_routes.labbillList); // Update to your lab bill list route
    } catch (error: any) {
      console.error("Error adding lab bill:", error.response?.data || error.message);
      alert(error.response?.data?.detail || error.message || "Failed to add lab bill");
    } finally {
      setLoading(false);
    }
  };
